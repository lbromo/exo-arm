...
a1  = 320e-3;        % [m] Distance from {0} to {1} along x0
...
%% Frictions 
cm= [0.0964    0.0072];%[0.066 0.0359]*N;%;[0.0000    0.0143 ];%[0.0253    0.0143 ];%[0.0429    0.0108];%[0.0660 0.0359];%[165.0942/(N^2) 89.7613/(N^2)];%[31.519/(N^2) 18.6925/(N^2)];%[224.8743/(N^2) 95.9248/(N^2)];%[283.27/(N^2) 110.16/(N^2)];%[119.9795/(N^2) 51.5666/(N^2)];
vm=[1.9469e-4 4.4047e-5]*N; %[0.0018    0.0000 ]*N;%[1.9469e-4 4.4047e-5]*N; %[1.9469e-4 4.4047e-5]; %[0.4867/(N^2) 0.1101/(N^2)];% [2.0377/(N^2) 1.0787/(N^2)];%[0.7988/(N^2) 0.274/(N^2)];%[0.9872/(N^2) 0.4129/(N^2)]; [1.2548/(N^2) 0.5650/(N^2)];
sigmoidpar=[0.0475    0.1299];%[0.1383 0.0889]; %[0.1383 0.0889]; %[0.1383 0.0889]; %0.0014    0.0109    5.9067    0.1078 0.0600    0.0099    0.0805    0.1069
hast=[3.4190    2.0148];

%0.0964    0.0072    0.0475    0.1299
%0.0455    0.0193    0.0001    0.0000    0.1042    0.0507


%0.0466    0.0186    0.1035    0.0513 
%0.0660 0.0359 1.9469e-4 4.4047e-5 0.1383 0.0889
